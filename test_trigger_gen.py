...
import sys
import datetime
from trigger_gen import parse_time_arg, parse_duration_arg, TriggerGenerator, RunningStats
...
class TestTriggerGen(unittest.TestCase):
    ...
    def test_running_stats(self):
        ...
        stats.add(0.001)
        stats.add(0.003)
        snapshot = stats.snapshot(scale=1000.0)

        self.assertEqual(snapshot["count"], 2)
        self.assertAlmostEqual(snapshot["mean"], 2.0, places=6)
        self.assertAlmostEqual(snapshot["min"], 1.0, places=6)
        self.assertAlmostEqual(snapshot["max"], 3.0, places=6)
    ...
    def test_generator_run(self, mock_gpiod, mock_time):
        ...
        # Alignment wait should include an initial ~1s wait to reach 1001.0.
        sleep_args = [call.args[0] for call in mock_time.sleep.call_args_list if call.args]
        self.assertTrue(any(s >= 0.9 for s in sleep_args))
